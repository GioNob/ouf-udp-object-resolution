...
| SBOM, vulnerability gate and deployment baseline | IMPLEMENTED BASELINE | CycloneDX image SBOM, fixed-critical Trivy gate, recorded tool/image IDs, checksums, kubeconform and hardened Kubernetes resources |
| Production recovery, capacity and release attestation | PARTIAL | CI baselines implemented; infrastructure acceptance, production sizing/SLO, registry signing, approved base digests, admission and rollout evidence remain open |
