...
class GovernedCrsTransformRuntimeTest {
 ...
 @Test void operationAreaAndGeometryLimitsAreEnforced(){assertThat(run(point("EPSG:4326",15,45),rule(4326,32631,"XY","CONVERT",forward(),backward())).status()).isEqualTo("SPATIAL_OUTSIDE_OPERATION_AREA");var legacy=new UdpPorts.GeometryRule("geometry","EPSG:4326",4326,"v1","OPEN");assertThat(run(point("EPSG:4326",181,45),legacy).status()).isEqualTo("SPATIAL_INVALID_GEOMETRY");assertThat(run(Map.of("crs","EPSG:4326","geoJson",Map.of("type","Point","coordinates",List.of(1,2,3))),legacy).status()).isEqualTo("SPATIAL_INVALID_GEOMETRY");}
