...
public class ObjectResolutionService {
  ...
  @Transactional public Decision resolve(String handoffId,Map<String,Object> payload,UdpPorts.ResolutionProfile profile){
    List<Map<String,Object>> prior=db.sql("select outcome,target_urban_object_id from ouf_udp.resolution_decision where handoff_id=:h").param("h",handoffId).query().listOfRows();if(!prior.isEmpty())return new Decision(String.valueOf(prior.getFirst().get("outcome")),(UUID)prior.getFirst().get("target_urban_object_id"),true);
