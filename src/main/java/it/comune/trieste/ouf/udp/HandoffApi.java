...
public class HandoffApi {
  ...
  @GetMapping("/{handoffId}") Map<String,Object> get(@PathVariable String handoffId,HttpServletRequest request){authorization.require(request,"udp.candidate.write",null,null);var row=service.handoff(handoffId);authorization.require(request,"udp.candidate.write",String.valueOf(row.get("source_id")),String.valueOf(row.get("ingestion_run_id")));return row;}
}
