...
final class WeightedIdentityCandidates {
  ...
  WeightedIdentity.Outcome resolve(Map<String,Object> incoming, UdpPorts.ResolutionProfile profile,
    ...
    if (spatial) query += " left join ouf_udp.urban_geometry_active gc on gc.urban_object_id=o.urban_object_id left join ouf_udp.urban_geometry g on g.geometry_revision_id=gc.geometry_revision_id cross join (select ST_GeomFromEWKB(decode(:geometry,'hex')) geom) s";
