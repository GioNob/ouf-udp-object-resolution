...
public class PublishedResolutionLoop {
  ...
  private final ResolutionRepository jobs;private final PublishedRuntimeConfiguration configuration;private final MaterializationReferenceGate gate;private final ObjectResolutionService resolution;private final CanonicalMaterializer materializer;private final JdbcClient db;private final TransactionTemplate tx;
  private final String worker="published-resolution-"+UUID.randomUUID();
  public PublishedResolutionLoop(ResolutionRepository jobs,PublishedRuntimeConfiguration configuration,MaterializationReferenceGate gate,ObjectResolutionService resolution,CanonicalMaterializer materializer,JdbcClient db,TransactionTemplate tx){this.jobs=jobs;this.configuration=configuration;this.gate=gate;this.resolution=resolution;this.materializer=materializer;this.db=db;this.tx=tx;}
  ...
  public void tick(){
    ...
    try{
      ...
      tx.executeWithoutResult(status->{
        ...
        if(Set.of("MATCH","NEW_OBJECT").contains(decision.outcome()))materializer.materialize(claim.handoffId(),decision.targetUrbanObjectId(),claim.payload(),profiles.materialization());
        jobs.complete(claim);
      });
